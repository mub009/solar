...
class Api_Supervisor_Controller extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        if (empty($this->input->get_request_header($auth_name, true))) {
            ...
            $DataInfo =  $this->authorization_token->ValidateToken($this->input->get_request_header($auth_name, true),$this->config->item('api_supervisor_jwt_key'),$this->config->item('api_jwt_algorithm'),0);

           

            if ($DataInfo['status'] == 1) {

                if ($DataInfo['data']->UserMasterId == 55) {
